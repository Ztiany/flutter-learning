...
//
//    1.1 在 Dart 中，实例变量与实例方法、类变量与类方法的声明与 Java 类似。Dart 中并没有 public、protected、private 这些关键字，
//        我们只要在声明变量与方法时，在前面加上“_”即可作为 private 方法使用。如果不加“_”，则默认为 public。
//
//    1.2 不过，“_”的限制范围并不是类访问级别的，而是库访问级别。
void classSample() {
  _defineClassSample();
  _reuseSample();
}
...
//
//    继承歧义，也叫菱形问题，是支持多继承的编程语言中一个相当棘手的问题。当 B 类和 C 类继承自 A 类，而 D 类继承自 B 类和 C 类时会产生歧义。
...
关于 with 的解释：
  The with keyword indicates the use of a "mixin". See here.
      ...
      A mixin refers to the ability to add the capabilities of another class or classes to your own class, without inheriting from those classes.
      The methods of those classes can now be called on your class, and the code within those classes will execute. Dart does not have multiple
      inheritance, but the use of mixins allows you to fold in other classes to achieve code reuse while avoiding the issues that multiple
      inheritance would cause.

      you can think of a mixin as an interface that lets you not merely specify that a given class will contain a given method, but also provide
