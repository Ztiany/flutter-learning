...
/*
 * 在 Dart 中，所有类型都是对象类型，函数也是对象，它的类型叫作 Function。这意味着
 * 函数也可以被定义为变量，甚至可以被定义为参数传递给另一个函数。
 *
 * C++ 与 Java 的做法是，提供函数的重载，即提供同名但参数不同的函数。但 Dart 认为重
 * 载会导致混乱，因此从设计之初就不支持重载，而是提供了可选命名参数和可选参数。
 */
void functionSample() {
  _sample1_defineFunc();
  _sample2_theParams();
